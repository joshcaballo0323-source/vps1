...
const token = "thr1.AAAAAGik4GbkinnQf8Xpgg.hfWu81b7uOM"; // https://www.haxball.com/headlesstoken
...
var errorWebhook = 'https://discord.com/api/webhooks/1407429834635739146/GnkY6_p1yzjSX8zgWYddjLBntegvBHrwaWu1YgffH3QIERq6eWXxTW7VucgAR_Ko78aU'
...
class WebhookQueue {
    constructor(webhook, username, interval = 1000) {
        ...
        this.processInterval = setInterval(() => this.process(), this.interval);
    }
    ...
    async process() {
        ...
        try {
            ...
            if (!response.ok) {
                console.error(`Error de sistema al enviar webhook: ${response.status} - ${response.statusText}`);
                this.isOperational = false;
                clearInterval(this.processInterval);
            }
        } catch (e) {
            console.error('Error de red al enviar mensaje al webhook:', e);
            this.isOperational = false;
            clearInterval(this.processInterval);
        } finally {
            this.isProcessing = false;
    ...
    send(player, message, hideClaimMessage = false) {
        if (!this.isOperational || !this.webhook) {
            return;
        }

        if (hideClaimMessage && message.startsWith('!claim')) {
            return;
        }

        const content = `[${getDate()}] 💬 CHAT\n**${player.name}** : ${message.replace('@', '@ ')}`;
...
class CooldownManager {
    ...
    addToCooldown(playerId, command, durationMs) {
        if (!this.cooldowns[command]) this.cooldowns[command] = {};
        this.cooldowns[command][playerId] = Date.now() + durationMs;
    }

    cleanExpiredCooldowns() {
        const now = Date.now();
        for (const command in this.cooldowns) {
            for (const playerId in this.cooldowns[command]) {
                if (this.cooldowns[command][playerId] <= now) {
                    delete this.cooldowns[command][playerId];
                }
            }
            if (Object.keys(this.cooldowns[command]).length === 0) {
                delete this.cooldowns[command];
            }
        }
    }

    canUse(playerId, command) {
        this.cleanExpiredCooldowns();

        if (!this.cooldowns[command]) return true;
        return !this.cooldowns[command][playerId] || this.cooldowns[command][playerId] <= Date.now();
    }

    timeLeft(playerId, command) {
        this.cleanExpiredCooldowns();

        if (!this.cooldowns[command] || !this.cooldowns[command][playerId]) return 0;
        return Math.max(this.cooldowns[command][playerId] - Date.now(), 0);
    }

    timeLeftString(playerId, command) {
        const ms = this.timeLeft(playerId, command);
        if (ms === 0) return "0s";
    ...
    clearPlayerCooldowns(playerId) {
        for (const command in this.cooldowns) {
            if (this.cooldowns[command][playerId]) {
                delete this.cooldowns[command][playerId];
            }
        }
    }
...
function leaveCommand(player, message) {
    if (player.team !== Team.SPECTATORS && players.length >= 2 * teamSize && gameState != State.STOP) {
...
function unicomando(player) {
    ...
    if (comandoUsadoEstaPartida) {
        room.sendAnnouncement(
            `⚠️ El comando !uniforme ya se ha usado en esta partida.`,
            player.id,
            errorColor,
            'normal',
            HaxNotification.CHAT
...
function skipCommand(player) {
    const playerId = authArray[player.id][1];
    const cooldownMs = 12 * 60 * 1000; // 12 Minutos
    ...
    if (!commandCooldowns.canUse(playerId, "skip")) {
        const timeString = commandCooldowns.timeLeftString(playerId, "skip");
        room.sendAnnouncement(
    ...
    commandCooldowns.addToCooldown(playerId, "skip", cooldownMs);
...
function banquearJugador(player, message) {
    ...
    if (!isVIP(player)) {
        room.sendAnnouncement(`💸 ${player.name}, necesitas ser VIP para usar este comando.`, player.id, 0x46A985, "bold", HaxNotification.CHAT);
        return;
    }

    if (!commandCooldowns.canUse(playerId, "banquear")) {
        const timeString = commandCooldowns.timeLeftString(playerId, "banquear");
        room.sendAnnouncement(`⏰ Por favor, espera ${timeString} antes de usar este comando nuevamente.`, player.id, 0xFFC857, "bold", HaxNotification.NONE);
        return;
    }
    ...
    if (player.id !== captain?.id) {
        room.sendAnnouncement(`¡Solo el capitán del equipo puede usar este comando!`, player.id, errorColor, "bold", HaxNotification.CHAT);
        return;
    }
    ...
    if (Object.keys(teamUsos).length > 0) {
        room.sendAnnouncement(`🔴 Tu equipo ya ha usado este comando en esta partida.`, player.id, errorColor, "bold", HaxNotification.CHAT);
        return;
    }
    ...
    if (!canUse) {
        room.sendAnnouncement(`⏰ Este comando solo se puede usar durante los primeros 30 segundos de la partida.`, player.id, errorColor, "bold", HaxNotification.CHAT);
        return;
    }
    ...
    if (!targetPlayer || targetPlayer.id === player.id || player.team !== targetPlayer.team || targetPlayer.team === Team.SPECTATORS) {
        room.sendAnnouncement(`Comando inválido o el jugador no está en el mismo equipo.`, player.id, errorColor, "bold", HaxNotification.CHAT);
        return;
    }
    ...
    commandCooldowns.addToCooldown(playerId, "banquear", cooldownMs);
    teamUsos[playerId] = true;
    room.setPlayerTeam(targetPlayer.id, Team.SPECTATORS);
    room.sendAnnouncement(`${player.name} ha banqueado a ${targetPlayer.name} usando su comando VIP.`, null, announcementColor, "bold", HaxNotification.CHAT);
    updateTeams();
    room.pauseGame(true);
}
...
room.onPlayerChat = function (player, message) {
    ...
    if (webhookQueue.isOperational) {
        webhookQueue.send(player, sanitizedMessage, hideClaimMessage);
    }
...
const sendErrorToWebhook = (title, details) => {
    if (!errorWebhook) {
        console.error("URL del webhook de errores no definida.");
        return;
    }
    const content = `🚨 ${title}\n${details}`;
    fetch(errorWebhook, {
        method: 'POST',
        body: JSON.stringify({ content, username: 'Error Logger' }),
        headers: { 'Content-Type': 'application/json' },
    }).catch(error => {
        console.error('Error al enviar el mensaje de error al webhook:', error);
    });
};

window.onerror = function (message, source, lineno, colno, error) {
    const errorDetails = `**Mensaje:** \`${sanitizeText(message)}\`\n` +
        `**Fuente:** \`${sanitizeText(source)}\`\n` +
        `**Línea:** ${lineno}, **Columna:** ${colno}\n` +
        `**Stack Trace:** \`\`\`${sanitizeText(error?.stack || 'No disponible')}\`\`\``;
    sendErrorToWebhook('ERROR GLOBAL', errorDetails);
    return true;
};

window.onunhandledrejection = function (event) {
    const reason = event.reason;
    let errorDetails = `**Razón:** \`${sanitizeText(reason)}\`\n`;
    if (reason instanceof Error) {
        errorDetails += `**Mensaje:** \`${sanitizeText(reason.message)}\`\n` +
            `**Stack Trace:** \`\`\`${sanitizeText(reason.stack)}\`\`\``;
    } else {
        errorDetails += `**Detalles:** ${sanitizeText(JSON.stringify(reason))}`;
    }
    sendErrorToWebhook('PROMESA RECHAZADA', errorDetails);
