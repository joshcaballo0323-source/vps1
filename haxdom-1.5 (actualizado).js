...
const token = "thr1.AAAAAGiniO4xhBmUMQ4qWw.eLGoKQu3SUo"; // https://www.haxball.com/headlesstoken
...
class WebhookQueue {
    ...
    send(player, message, hideClaimMessage = false) {
        ...
        if (hideClaimMessage && message.startsWith('!claim')) return;
...
function handleActivityPlayer(player) {
    ...
    if (pComp.inactivityTicks >= 60 * afkLimit) {
        ...
        room.sendAnnouncement(
            `🚪 ${player.name} ha sido removido por inactividad.`,
            null,
            0x87CEEB,
            'bold',
            HaxNotification.CHAT
...
room.onPlayerLeave = function (player) {
    ...
    //  Guadar posicion de jugador para otro //
    ///////////////////////////////////////////
...
room.onPlayerKicked = function (kickedPlayer, reason, ban, byPlayer) {
    ...
    votekickSystem.onPlayerLeave(kickedPlayer);
};
...
function handleCustomAvatar(player, message) {
    const avatarMap = {
        "q": "😲",
        "1": "😱",
        "ez": "🥱",
...
function playGoalAnimation(scoringTeam) {
    const { scorerId, assistId, rivalGKId } = getGoalIds(scoringTeam);
    function getPlayerStatsAvatar(playerId, type) {
        const stored = localStorage.getItem(authArray[playerId]?.[0]);
        if (!stored) return null;
        const stats = JSON.parse(stored);
        let value = type === 'goal' ? stats.goals : stats.assists;
    ...
    function animatePlayer(playerId, baseAvatars, statsAvatar) {
        ...
        let currentFrame = 0;
        function frame() {
            if (currentFrame < avatars.length) {
                room.setPlayerAvatar(playerId, avatars[currentFrame].avatar);
                currentFrame++;
    ...
    if (scorerId) {
        const scorerPlayer = room.getPlayerList().find(p => p.id === scorerId);
        if (scorerPlayer) {
            const radius = isVIP(scorerPlayer) ? 60 : 30;
            room.setPlayerDiscProperties(scorerId, { radius });
            const statsAvatar = getPlayerStatsAvatar(scorerId, 'goal');
            const scorerAnimation = [
                { avatar: "⚽" },
                { avatar: "👽" },
                { avatar: "🦿" }
            ];
            animatePlayer(scorerId, scorerAnimation, statsAvatar);
        }
    }
    if (assistId) {
        const assistPlayer = room.getPlayerList().find(p => p.id === assistId);
        if (assistPlayer) {
            room.setPlayerDiscProperties(assistId, { radius: 20 });
            const statsAvatar = getPlayerStatsAvatar(assistId, 'assist');
            const assistAnimation = [
                { avatar: "👟" },
                { avatar: "🧏‍♀️" },
                { avatar: "👀" }
            ];
            animatePlayer(assistId, assistAnimation, statsAvatar);
        }
    }
    if (rivalGKId) {
        const gkPlayer = room.getPlayerList().find(p => p.id === rivalGKId);
        if (gkPlayer) {
            ...
            animatePlayer(rivalGKId, gkAnimation, null);
        }
    }
}
...
var announcements = [
    ...
    "Usa !afk solo en los primeros 30s de la partida si estás en cancha.\nSi no estás en cancha, no hay límite.",
    "Recuerda que tu ELO añadido después de una partida depende de tu rendimiento y contribuciones.",
    ...
    "¿Quieres cambiar tu uniforme o iniciar votación?\nUsa !uni y comienza votación o cambia si eres capitán",
    ...
    "Consulta los VIPs actuales con !vips\nRecuerda que puedes estar aqui igual.. !vip",
    "Obtén el link del Discord con !discord\nÚnete a la comunidad y participa en eventos y la liga",
    "Usa el comado !votekick para iniciar una votacion, el mal uso será sancionado",
    "Usa el comado !vote para votar en la votacion actual, votar en votekicks indebidos serán sancionados"
];
...
function formatMessage(message) {
    ...
    if (lines.length === 2 && lines[0].length > lines[1].length) {
        lines[0] = lines[0].slice(0, Math.floor(lines[0].length * 0.9));
    }
    return lines.join("\n");
}
...
setInterval(() => {
    ...
    formattedMessage.split("\n").forEach(line => {
        room.sendAnnouncement(line, null, 0xFFFF00, 'normal', HaxNotification.CHAT);
    });

    announcementCounter++;
    if (announcementCounter % 3 === 0 && typeof discordlink !== "undefined") {
        discordlink.split("\n").forEach(line => {
            room.sendAnnouncement(line, null, 0xFFFF00, 'normal', HaxNotification.CHAT);
        });
    }
